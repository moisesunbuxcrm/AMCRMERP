...
/**
 *	Class to generate a standard PDF Production Order
 */
class pdf_po_standard extends pdf_po_base
{
	var $po;
	...
	{
		return new pdf_po_standard_impl($this->db, $this->po, $outputlangs);
	}
	...
	function setFilename()
	{
		...
		else
		{
			...
			$this->dir = $conf->amhpestimates->dir_output . "/po-" . $ref;
			$this->filename = "po-" . $ref . ".pdf";
			$this->filepath = $this->dir . "/" . $this->filename;
		}
	}
}

class pdf_po_standard_impl extends pdf_po_base_impl 
{
	var $po;
	var $estimateNumberR;
	var $summaryR, $validityR;
	...
	function __construct($db, $po, $outputlangs)
	{
		...
		$this->SetTitle($this->outputlangs->convToOutputCharset("Estimate ".$po->PONUMBER));
	...
	function generate()
	{
		...
		$midpoint_margin = 2;
		$column_width = $midpointx-$midpoint_margin/2-$this->pageR->x;
		$right_col_start = $midpointx+$midpoint_margin/2;

		$this->AddEstimateNumber($right_col_start, $this->headerR->GetBottom(), $column_width, 0);
		$this->AddColor($this->po, $right_col_start, $this->estimateNumberR->GetBottom()+3.5, $column_width, 0);
		$this->AddDocumentLabelAndDate("Quote Date", $this->po->QUOTEDATE, $this->pageR->x, $this->headerR->GetBottom(), $column_width, 0);
		...
		$itemHeaders = array(
			...
			array('prop'=>'SQFEETPRICE','width'=>.0625, 'format'=>function($val) { return round((float)$val, 2); }),
			array('prop'=>'INSTFEE','width'=>.0625, 'format'=>function($val) { return round((float)$val, 2); }),
		...
		$this->AddItemsArea($this->po, $itemHeaders, $this->pageR->x, $this->vendorInfoR->GetBottom()+$this->GetFontSize(), $this->pageR->w, 0);
		...
		$this->AddSummary($this->pageR->x + $this->pageR->w/4, $this->observationR->GetBottom(), $this->GetFontSize(), $this->pageR->w*3/4, 0);
		$this->AddValidityNote($this->pageR->x, $this->summaryR->GetBottom()+$this->GetFontSize(), $this->GetFontSize(), $this->pageR->w, 0);

		$this->Close();
	}

	public function AddEstimateNumber($x, $y, $w, $h)
	{
		$this->SetXY($x,$y);
		...
		$this->SetFont($this->getFontFamily(), 'B', $this->getFontSizePt());
		 $this->SetX($x+22);
		$this->Cell($column_width, $h, "Estimate No.:", $this->draw_borders, 0, "L");
		$this->SetX($x+39);
		...
		$this->writeHTMLCell($column_width, 0, $x+24, $y+4 , "License No.: ", 0, 1, false, true, 'L');
        $this->writeHTMLCell($column_width, 0, $x+49, $y+4 , $val, 0, 1, false, true, 'L');
		$this->writeHTMLCell($column_width, 0, $x+29, $y+8 , "Folio No.: ", 0, 1, false, true, 'L');
		$this->SetDefaultFont();
		$this->writeHTMLCell($column_width+10, 0, $x+49, $y+8 , $barcode,   0, 1, false, true, 'L');
	}
	...
	public function AddItemsHeader($itemHeaders, $x, $y, $w, $h)
	{
		...
		$xx = $this->GetX();
		$this->Cell($itemHeaders[$c]['width']*$w*2, $headerHeight/2, "Finish Size", 1, 0, 'C');
		$this->Ln($headerHeight/2);
		$this->setX($xx);
		$this->Cell($itemHeaders[$c++]['width']*$w, $headerHeight/2,  "W", 1, 0, 'C');
		$this->Cell($itemHeaders[$c++]['width']*$w, $headerHeight/2, "HT", 1, 0, 'C');
	...
	public function AddSummary($x, $y, $gap, $w, $h)
	{
		...
		$this->SetFont(pdf_getPDFFont($this->outputlangs),'B',pdf_getPDFFontSize($this->outputlangs));
		$this->Cell($cw[$c++]*$w, 0, "Discount:", 0, 0, 'R');
		$this->SetDefaultFont();
		$this->Cell($cw[$c++]*$w, 0, "$".number_format($this->po->Discount, 2), 0, 1, 'R');
		...
		$this->SetDefaultFont();
		$this->Cell($cw[$c++]*$w, 0, round($this->po->TOTALALUM*1000)/1000, 0, 0, 'R');
		$this->SetFont(pdf_getPDFFont($this->outputlangs),'B',pdf_getPDFFontSize($this->outputlangs));
		...
		$c=0;
		$this->SetFont(pdf_getPDFFont($this->outputlangs),'B',pdf_getPDFFontSize($this->outputlangs));
		$this->Cell($cw[$c++]*$w, 0, "Other Fees :", 0, 0, 'R');
		$this->SetDefaultFont();
		$this->Cell($cw[$c++]*$w, 0, "$".number_format($this->po->OTHERFEES, 2), 0, 0, 'R');
		$this->SetFont(pdf_getPDFFont($this->outputlangs),'B',pdf_getPDFFontSize($this->outputlangs));
	...
	public function AddValidityNote($x, $y, $gap, $w, $h)
	{
		...
		$this->writeHTMLCell($this->pageR->w, 0, $x, $y, "There will be a 3% charge for using a credit card<br/>This price is valid for 30 days", 0, 0, false, true, 'C');
