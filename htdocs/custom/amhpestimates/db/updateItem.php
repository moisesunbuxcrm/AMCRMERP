...
$id = GETPOST("PODescriptionID", 'int');

if (!$id)
{
  echo '{ "msg": "update(): Missing or empty PODescriptionID" }';
}
else
{
  ...
  if ($itemDB->fetch($id))
  {
    ...
    $itemDB->PRODUCTTYPE = GETPOST("PRODUCTTYPE", 'int');
    ...
    if ($itemDB->update() == 1)
      ...
      echo '{ "msg": "Update failed: ' . $itemDB->error . '" }';
  }
  else
  {
    if ($itemDB->error)
      ...
      echo '{ "msg": "Update failed: Unknown PODescriptionID=' . $id . '" )';
  }
}
