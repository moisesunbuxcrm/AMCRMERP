...
/**
 *	This class is a CRUD wrapper for accessing the llx_ea_est_design table
 */
class EaEstimateItemDesign extends CommonObject
{
	var $db;							//!< To store db handler
	var $error;							//!< To return error code (or message)
	var $errors=array();				//!< To return several error codes (or messages)
	var $element='eaestdesign';			//!< Id that identify managed objects
	var $table_element='ea_est_design';		//!< Name of table without prefix where object is stored
	var $specimen;
	...
	function create()
	{
		...
		{
			// Insert request
			$sql = "INSERT INTO ".MAIN_DB_PREFIX."ea_est_design (";
	...
	/**
	 *  Duplicate an design item
	 *
	 *  @param	int		$id	Id object
	 *  @return int		The id of the design item to copy
	 */
	function duplicate($id, $newItemId)
	{
		global $langs;
		$newid = 0;
		$sql = "CALL copy_estimate_item_design(".$id.",".$newItemId.")";
	...
	function update()
	{
		...
		// Update request
		$sql = "UPDATE ".MAIN_DB_PREFIX."ea_est_design SET";
	...
	function delete($user)
	{
		...
		if (! $error)
		{
			...
			{
				$sql = "DELETE FROM ".MAIN_DB_PREFIX."ea_est_design";
				$sql.= " WHERE id=".$this->id;
	...
	function createFromClone($fromid)
	{
		...
		$object=new EaEstimateItemDesign($this->db);
	...
	function initAsSpecimen()
	{
		...
		$sql = "SELECT";
		$sql.= " max(e.id) as id";		
		$sql.= " FROM ".MAIN_DB_PREFIX."ea_est_design as e";
	...
	function fetchAllById($ids)
	{
		...
		foreach ($ids as $id)
		{
			$e = new EaEstimateItemDesign($this->db);
			$result = $e->fetch($id);
			if ($result<0)
			{
				$this->error="Error ".$this->db->lasterror();
