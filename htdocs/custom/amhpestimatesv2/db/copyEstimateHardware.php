...
require_once DOL_DOCUMENT_ROOT.'/custom/amhpestimatesv2/class/eaestimate_item_hardware.class.php';
...
{
  $eii = new EaEstimateItemHardware($db);
  if ($eii->fetch($copyID)) 
  {
    ...
    if ($newItemId > 0)
    {
      $newHardwareId = $eii->duplicate($copyID, $newItemId);
      if ($newHardwareId > 0)
      {
        echo '{ "msg": "OK", "ID": ' . $newHardwareId . ' }';
      }
      else
      {
