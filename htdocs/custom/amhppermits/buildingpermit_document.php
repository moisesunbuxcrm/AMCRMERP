...
/**
 *  \file       buildingpermit_document.php
 *  \ingroup    amhppermits
 *  \brief      Tab for documents linked to BuildingPermit
 */
...
$langs->loadLangs(array("amhppermits@amhppermits","companies","other"));
...
$hookmanager->initHooks(array('buildingpermitdocument'));     // Note that conf->hooks_modules contains array
...
$title=$langs->trans("BuildingPermit").' - '.$langs->trans("Files");
$help_url='';
...
if ($object->id)
{
	...
	dol_fiche_head($head, 'document', $langs->trans("BuildingPermit"), -1, 'buildingpermit@amhppermits');
	...
	$modulepart = 'amhppermits';
	//$permission = $user->rights->amhppermits->create;
	$permission = 1;
	//$permtoedit = $user->rights->amhppermits->create;
	$permtoedit = 1;
	$param = '&id=' . $object->id;

	//$relativepathwithnofile='buildingpermit/' . dol_sanitizeFileName($object->id).'/';
	$relativepathwithnofile='buildingpermit/' . dol_sanitizeFileName($object->ref).'/';
