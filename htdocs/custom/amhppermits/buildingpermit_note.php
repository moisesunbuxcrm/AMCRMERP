...
/**
 *  \file       buildingpermit_note.php
 *  \ingroup    amhppermits
 *  \brief      Car with notes on BuildingPermit
 */
...
$langs->loadLangs(array("amhppermits@amhppermits","companies"));
...
$hookmanager->initHooks(array('buildingpermitnote'));     // Note that conf->hooks_modules contains array
...
if ($id > 0 || ! empty($ref)) $upload_dir = $conf->amhppermits->multidir_output[$object->entity] . "/" . $object->id;

$permissionnote=1;
//$permissionnote=$user->rights->amhppermits->creer;	// Used by the include of actions_setnotes.inc.php
...
$help_url='';
llxHeader('',$langs->trans('BuildingPermit'),$help_url);
...
if ($id > 0 || ! empty($ref))
{
	...
	dol_fiche_head($head, 'note', $langs->trans("BuildingPermit"), -1, 'buildingpermit@amhppermits');
