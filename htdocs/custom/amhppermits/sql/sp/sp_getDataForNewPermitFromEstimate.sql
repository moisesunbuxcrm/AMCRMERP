DROP PROCEDURE IF EXISTS `getDataForNewPermitFromEstimate`;
--
CREATE PROCEDURE `getDataForNewPermitFromEstimate`(IN `eid` INT)
    READS SQL DATA
SELECT 
        tp.barcode as ref, tp.nom as label, tp.rowid as fk_soc,
        ...
        getConst('MAIN_INFO_SOCIETE_TEL') as contractorphone, getConst('MAIN_INFO_SOCIETE_FAX') as contractorfax, getConst('AMHP_DEFAULT_CURRENT_USE') as currentuse, 
        getConst('AMHP_DEFAULT_CURRENT_USE') as proposeduse, 
        'INSTALLATION OF' as descriptionofwork, 'INSTALLATION OF' as description_of_improvements, 0 as totalalum, 0 as totallinearft,
        NULL as units, NULL as floors, e.totalprice as valueofwork, 
        ...
        e.permits as permitfee,  getConst('AMHP_DEFAULT_COMMISION_NUMBER') as commission_number, 'RESIDENTIAL' as classificationofwork
    FROM 
         llx_societe tp LEFT JOIN 
         llx_ea_estimate e on e.customerId = tp.rowid LEFT JOIN
