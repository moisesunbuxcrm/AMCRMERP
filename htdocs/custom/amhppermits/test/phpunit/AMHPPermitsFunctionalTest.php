...
/**
 * \file    test/functional/AMHPPermitsFunctionalTest.php
 * \ingroup amhppermits
...
/**
 * Class AMHPPermitsFunctionalTest
 *
 ...
 *
 * @package Testamhppermits
 */
class AMHPPermitsFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
...
//        )
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modAMHPPermits')->click();
		} else {
			// Disable the module
			$this->byHref('modAMHPPermits')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/amhppermits/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('amhppermits/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/amhppermits/admin/about.php');
		$this->authenticate();
		return $this->assertContains('amhppermits/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('amhppermitswidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modAMHPPermits_AMHPPermitsTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modAMHPPermits_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
