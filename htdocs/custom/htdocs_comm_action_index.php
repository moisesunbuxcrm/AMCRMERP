...
if (empty($action) || $action == 'show_month') {
	...
	$max_day_in_prev_month = date("t", dol_mktime(12, 0, 0, $prev_month, 1, $prev_year, 'gmt')); // Nb of days in previous month //pdermody
	$max_day_in_month = date("t", dol_mktime(12, 0, 0, $month, 1, $year)); // Nb of days in next month //pdermody
	// tmpday is a negative or null cursor to know how many days before the 1st to show on month view (if tmpday=0, 1st is monday)
	$tmpday = -date("w", dol_mktime(12, 0, 0, $month, 1, $year, 'gmt')) + 2; // date('w') is 0 fo sunday //pdermody
...
if ($resql) {
	...
	while ($i < $num && $i < $MAXONSAMEPAGE) {
		...
		if ($event->date_end_in_calendar < $firstdaytoshow || $event->date_start_in_calendar >= $lastdaytoshow)	{
			...
			$loop = true; $j = 0;
			$daykey = dol_mktime(12, 0, 0, $mois, $jour, $annee, 'gmt'); //pdermody
			do {
				//if ($event->id==408)
